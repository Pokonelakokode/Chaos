...
export const initialPlayer: IPlayer = {
    ...
    creatures: [],
    spells: [],
    character: ICharacterTypes.GREEN,
    movementPoints: 0,
    attackPoints: 0,
    stats: {...initialStats}
}
    ...
    switch (action.type) {
        ...
        case PlayerActionTypes.STEP:
            return state.map((player, id) => {
                if (id !== action.id) return player;
                return {
                    ...player,
                    col: player.col! + action.col,
                    row: player.row! + action.row,
                }
            });
        case PlayerActionTypes.REMOVE_MOVE_POINT:
            state[action.id].movementPoints = state[action.id].movementPoints - action.points;
            return state;
